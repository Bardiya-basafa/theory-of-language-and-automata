...
try:
    ...
    print(
        "Make sure turing_machine.py exists in the same folder and contains a class named TuringMachine."
    )
    sys.exit(1)
...
def test_generator_structure():
    ...
    try:
        ...
        assert (
            len(steps) >= 2
        ), "Generator should yield at least two steps (initial and final)"

        first_step = steps[0]
        assert (
            isinstance(first_step, tuple) and len(first_step) == 2
        ), "Each step must be a tuple of (action, configuration)"

        action, config = first_step
        assert (
            action is None
        ), "First action should be None as the machine is still running"

        expected_keys = {"state", "left_hand_side", "symbol", "right_hand_side"}
        assert isinstance(config, dict) and expected_keys.issubset(
            config.keys()
        ), f"Configuration should be a dictionary with keys: {expected_keys}"

        print("✅ Generator yielded correctly structured tuples.")
        print(
            "✅ Configuration dictionary matches the unified interface specification."
        )
...
if __name__ == "__main__":
    ...
    if t1 and t2 and t3:
        print(
            "🎉 CONGRATULATIONS! Your TuringMachine simulator passes all basic self-tests."
        )
        print(
            "Ready for Part II and Part III! Next, try running the provided examples."
        )
    else:
        print(
            "❌ Warning: Some self-tests failed. Please check your implementation in turing_machine.py."
        )
    print("==================================================")
